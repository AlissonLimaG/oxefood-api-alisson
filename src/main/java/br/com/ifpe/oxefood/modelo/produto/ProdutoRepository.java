...
public interface ProdutoRepository extends JpaRepository<Produto, Long> {
    ...
    @Query(value = "SELECT p FROM Produto p WHERE p.categoriaProduto.id = :idCategoria")
    List<Produto> consultarPorCategoria(Long idCategoria);

    // Exemplo de uma busca com mais de um atributo
    @Query(value = "SELECT p FROM Produto p WHERE p.titulo ilike %:titulo% AND p.categoriaProduto.id = :idCategoria")
